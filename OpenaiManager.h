...
public:
    ...
    // 发送消息到OpenAI API
    QString sendMessage(const QString &url, const QString &apiKey, const QString &model,
                        const QString &prompt,
                        const QVector<QString> &userPrompts      = QVector<QString>(),
                        const QVector<QString> &assistantPrompts = QVector<QString>());

    // 测试API连接
    bool testApi(const QString &url, const QString &apiKey, const QString &model);
...
private:
    ...
    // 发送HTTP请求到OpenAI API
    QString sendRequest(const QString &jsonData, const QString &url, const QString &apiKey);
