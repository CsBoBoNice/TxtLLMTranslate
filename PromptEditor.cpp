...
PromptEditor::PromptEditor(FilePrompt *filePrompt, QWidget *parent)
    : QDialog(parent), m_filePrompt(filePrompt)
{
...
void PromptEditor::initConnections()
{
    ...
    connect(m_addAssistantPromptBtn, &QPushButton::clicked, this,
            &PromptEditor::addAssistantPrompt);
    connect(m_editAssistantPromptBtn, &QPushButton::clicked, this,
            &PromptEditor::editAssistantPrompt);
    connect(m_removeAssistantPromptBtn, &QPushButton::clicked, this,
            &PromptEditor::removeAssistantPrompt);
...
{
    bool ok;
    QString text = QInputDialog::getMultiLineText(this, "添加用户提示", "请输入提示内容:", "", &ok);
    if (ok && !text.isEmpty())
    {
...
{
    bool ok;
    QString text = QInputDialog::getMultiLineText(this, "添加助手提示", "请输入提示内容:", "", &ok);
    if (ok && !text.isEmpty())
    {
